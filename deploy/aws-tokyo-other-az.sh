...
# Run this on the second AWS Tokyo comparison instance.
# It can be in any ap-northeast-1 AZ; the dashboard will mark whether it is the
# same AZ ID as Extended (apne1-az4) or a different AZ.
...
NODE_LABEL="${NODE_LABEL:-aws-tokyo-other-az}"
...
EXTENDED_FILL_TEST_PRICE_OFFSET_PCT="${EXTENDED_FILL_TEST_PRICE_OFFSET_PCT:-0}"
...
if [ "$AZ_ID" = "$EXPECTED_AZ_ID" ]; then
  echo "NOTE: this comparison node is actually in Extended same AZ ID $EXPECTED_AZ_ID."
else
  echo "OK: comparison node AZ ID is $AZ_ID, Extended target AZ ID is $EXPECTED_AZ_ID."
fi
